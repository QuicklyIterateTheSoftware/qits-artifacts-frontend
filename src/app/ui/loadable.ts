 ...
 * app that distinction earns its keep on the repository page, where a `ci-screenshots` repository
 * has no listing endpoint at all — its table is idle forever and says so, rather than spinning on a
 * request nobody is going to make.
 *
 * Every panel holds its own, which is what lets the store summary fail to a small inline retry
 * while the repositories table beside it stays standing.
 *
 * Copied from spa-ci rather than shared, like `Async` and `Empty` below it: there is no place to
 * put it yet — @qits/ui-components carries presentational components, not application types.
 */
